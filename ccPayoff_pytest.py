...
def test_negative_ccBalance():
    assert BankFunctions.ccPayoff(-1000, 12, 2) == "CC balance cannot be negative."
    print("--Should reject a negative CC balance input--")

def test_negative_ccInterestRate():
    assert BankFunctions.ccPayoff(1000, -12, 2) == "CC interest rate cannot be negative."
    print("--Should reject a negative CC interest rate input--")

def test_negative_months():
    assert BankFunctions.ccPayoff(1000, 12, -2) == "Desired months to payoff cannot be negative."
    print("--Should reject a negative desired months to payoff input-- ")

def test_NaN_ccBalance():
    assert BankFunctions.ccPayoff("test", 12, 2) == "CC balance input value must be numeric."
    print("--Should reject a CC balance that is not a number--")

def test_NaN_ccInterestRate():
    assert BankFunctions.ccPayoff(1, "test", 2) == "CC interest rate input value must be numeric."
    print("--Should reject a CC interest rate that is not a number--")

def test_NaN_months():
    assert BankFunctions.ccPayoff(1, 12, "test") == "Desired months to payoff input value must be numeric."
...
def test_integer_calculation():
    ...
    assert BankFunctions.ccPayoff(1542.00, 5.2, 7) == d
